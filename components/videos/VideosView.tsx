...
export function VideosView({ profile, initialQuery, initialLanguage, onAddCard, userId }: Props) {
  ...
  return (
    <div className="videos-tab-container">
      ...
      {activeVideo && <VideoPlayerModal video={activeVideo} profile={profile} userId={userId} resumePositionSeconds={progress[activeVideo.youtubeId]?.position || 0} onClose={() => setActiveVideo(null)} onAddCard={onAddCard} onProgress={(current, duration, cueIndex, cueText) => {
