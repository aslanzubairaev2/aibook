...
- Pick ONE widget per exercise, by what the instruction actually asks for:
  - "cloze": items are given sentences with one or more gaps to fill (dots, underscores, an ellipsis, a blank space before a mark). Mark each gap "{{0}}", "{{1}}", ... in the item's "text", in order, and add one entry to "blanks" per gap.
    - A blank's "select" is true only when the exercise supplies a small fixed set of words that must fill EVERY gap in it (e.g. "Вставьте личные местоимения er, sie, es" — that set goes in "bank", every blank is select:true). Otherwise "select" is false (the learner writes freely — e.g. a verb ending, an article).
