...
export function extractPageLabel(...values: Array<string | null | undefined>): string {
  for (const value of values) {
    const match = value?.match(/(?:seite|s\.|стр\.?|страниц(?:а|е)?|page)\s*[:.]?\s*(\d+(?:\s*[-–]\s*\d+)?)/iu);
...
export function withPageDescription(description: string, pageLabel: string): string {
  ...
  if (!pageLabel || new RegExp(`(?:страниц|seite|page|стр\\.?)\\s*${pageLabel.replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")}`, "iu").test(trimmed)) {
    return trimmed;
  }
