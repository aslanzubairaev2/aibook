...
export const CAPABILITY_AREAS: CapabilityArea[] = [
  ...
  },
  {
    ...
    summary:
      "The learner's own dictionary: full entries (article, plural, verb forms, CEFR, example), grouped into packs. Every saved item has an explicit material type: word, phrase, sentence, or expression (a fixed expression, idiom, collocation or formula such as «Auf Wiederhören!»). A pack is one unit of study — a photographed coursebook page, or a themed set you built with them — with its own progress bar and its own «тренировать» button on the Словарь screen. Dictionary entries go in with add_word_batch; cards that do not need dictionary fields go in add_flashcards with a batch_title. Everything in a pack is a flashcard from the moment it is added. A pack can also carry its own training setup (update_batch_training): direction, material type, status, trainer — so «тренировать» opens it drilled the way that pack is meant to be drilled, while the learner's own filters stay the default everywhere else. And it carries what it is: update_pack_details writes a description shown under its title and the brief it was built to, which is the only thing that tells one shelf of noun packs from the next months later. delete_pack removes a pack and its entries when it turned out empty, wrong, or the learner asked for it gone — the cards it made stay in the deck unless you delete_flashcards them too.",
    tools: [
...
export const AGENT_TIPS: string[] = [
  "Write at the learner's level: pull their vocabulary with get_progress or get_study_words first, build the text mostly from confident words, and weave in a few they are forgetting.",
      "Anything you build as a set belongs in a pack, never in loose cards: dictionary words or expressions with article/forms/examples go in add_word_batch; cards for phrases, sentences or expressions without dictionary fields go in add_flashcards with a 'batch_title'. Always set the exact material type — word, phrase, sentence or expression — because it drives the filters in Словарь and in flashcard practice. The pack is what the learner can see on the Словарь screen, watch progress on, and train with one tap; loose cards are reachable only by hunting through a filter list.",
